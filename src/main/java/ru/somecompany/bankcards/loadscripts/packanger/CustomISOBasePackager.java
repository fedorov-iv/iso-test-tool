package ru.somecompany.bankcards.loadscripts.packanger;
...
import java.util.Map;
	 ...
	 * @author apr
	 */
	public abstract class CustomISOBasePackager implements ISOPackager, LogSource {
	    ...
	    public byte[] pack (ISOComponent m) throws ISOException {
	        ...
	        try {
	            ...
	            ISOComponent c;
	            ArrayList<byte[]> v = new ArrayList<byte[]>(152);
	            ...
	            int tmpMaxField=Math.min (m.getMaxField(), 152);
	            ...
	            if(m.getMaxField()>152 && fld.length > 152) {
	                for (int i=1; i<=64; i++) {
	                    if ((c = (ISOComponent) 
	                        fields.get (i + 152)) != null)
	                    {
	                        try {
	                            b = fld[i+152].pack(c);
	                            len += b.length;
	                            ...
	                            if (evt != null) {
	                                evt.addMessage ("error packing field "+(i+152));
	                                evt.addMessage (c);
	                                evt.addMessage (e);
	                            }
	                            throw e;
	        ...
	        throws IOException, ISOException 
	    {
	        ...
	        try {
	            ...
	            if (bmap != null && bmap.get(65) && fld.length > 152 &&
	                fld[65] instanceof ISOBitMapPackager)
	            {
	                bmap= (BitSet) ((ISOComponent) m.getChildren().get(65)).getValue();
	                for (int i=1; i<64; i++) {
	                    if (bmap == null || bmap.get(i)) {
	                        ISOComponent c = fld[i+152].createComponent(i);
	                        fld[i+152].unpack (c, in);
	                        if (logger != null) {
	                            evt.addMessage ("<unpack fld=\"" + i+152
	                                +"\" packager=\""
	                                +fld[i+152].getClass().getName()+ "\">");
	                            evt.addMessage ("  <value>" 
	                                +c.getValue().toString()
	                                + "</value>");
	                            evt.addMessage ("</unpack>");
